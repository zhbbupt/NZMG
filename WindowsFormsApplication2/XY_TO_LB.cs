    ...
    class XY_TO_LB
    {
        ...
        protected double[] TransformXYToBL()
        {
            ...
            for (int i = 0; i <= 5; i++)
            {
                ...
                complex_add(out Zeta, Zeta, tmp_Zeta);
                complex_multiplication(out tmp_z, tmp_z, z);
            }
            tmp_Zeta = Zeta;
            complex Zeta_2, tmp_B, Zeta_2_numerator, Zeta_2_denominator;
            tmp_B.real = B[0, 0];
            tmp_B.imag = B[0, 1];
            ...
            complex_multiplication(out tmp_Zeta, Zeta, Zeta);
            for (int i = 1; i <= 5; i++)
            {
                tmp_B.real = B[i, 0];
                tmp_B.imag = B[i, 1];
            ...
            for (int i = 0; i <= 8; i++)
            {
                ...
                tmp_phi = Zeta_2.real * tmp_phi;
            }
            double m_B, m_L;
            m_B = latitudeOrigin + (100000 * delta_phi) / (3600.0);
            m_L = longitudeOrigin + (Zeta_2.imag * 180.0) / p;
            double[] result = new double[2];
            result[0] = m_B;
            result[1] = m_L;
            return result;
